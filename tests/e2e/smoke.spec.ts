...
test("home renders brand and catalogue CTA", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { level: 1, name: /CLARKY/i })).toBeVisible();
  await expect(page.getByRole("link", { name: /Browse catalogue/i }).first()).toBeVisible();
});

test("catalogue and product pages are reachable", async ({ page }) => {
  await page.goto("/work");
  await expect(
    page.getByRole("heading", { level: 1, name: /Made-to-order prints/i }),
  ).toBeVisible();
